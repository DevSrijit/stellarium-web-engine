...
var testBasic = function (stel) {
  ...
  // Test Sun pos.
  {
    ...
    assert(isNear(dec, 6.2 * stel.D2R, 0.01));
  }
  ...
  // Test Polaris pos.
  {
    ...
    assert(isNear(dec, 89.3 * stel.D2R, 0.01));
...
var testListener = function (stel) {
  var test = 0;
  stel.observer.change("latitude", function () {
    test++;
  });
...
var testCalendar = function (stel) {
  ...
  var onEvent = function (ev) {
    if (
      ev.o2 &&
      ev.o1.designations().includes("NAME Moon") &&
...
var testVisibility = function (stel) {
  ...
  assert(polaris);
  assert(
    JSON.stringify(polaris.computeVisibility()) == '[{"rise":null,"set":null}]'
  );
  assert(sigauct);
...
var testTree = function (stel) {
  ...
  stel.onValueChanged(function (path, value) {
    ...
    if (path === "selection") assert(typeof value === "number");
  });
...
var testCreate = function (stel) {
  ...
  var obj4 = stel.createObj("star", {
    ...
    names: [
      ...
      "TYC 2369-193-1",
      "BD+36 805",
      "Gaia DR2 219547565555375488",
      "Gaia DR1 219547561257131776",
      "ROT 588",
      "WEB 3613",
      "GC 4809",
      "SKY# 6121",
