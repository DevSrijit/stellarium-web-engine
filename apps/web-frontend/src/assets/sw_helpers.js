...
const swh = {
  initStelWebEngine: function (store, wasmFile, canvasElem, callBackOnDone) {
    StelWebEngine({
      ...
      onReady: function (lstel) {
        ...
        Vue.prototype.$selectionLayer = lstel.createLayer({
          id: "slayer",
          z: 50,
          visible: true,
        });
        Vue.prototype.$observingLayer = lstel.createLayer({
          id: "obslayer",
          z: 40,
          visible: true,
        });
        Vue.prototype.$skyHintsLayer = lstel.createLayer({
          id: "skyhintslayer",
          z: 38,
          visible: true,
        });
  ...
  namesForSkySource: function (ss, flags) {
    ...
    res = res.concat(
      ss.names.map((n) => Vue.prototype.$stel.designationCleanup(n, flags))
    );
    ...
    if (context.$store.state.selectedObject) {
      link +=
        ...
        this.cleanupOneSkySourceName(
          context.$store.state.selectedObject.names[0],
          5
        ).replace(/\s+/g, "");
    }
    link += "?";
    link +=
      "fov=" + ((context.$store.state.stel.fov * 180) / Math.PI).toPrecision(5);
    const d = new Date();
    d.setMJD(context.$stel.core.observer.utc);
    link += "&date=" + new Moment(d).utc().format();
    link +=
      "&lat=" +
      ((context.$stel.core.observer.latitude * 180) / Math.PI).toFixed(2);
    link +=
      "&lng=" +
      ((context.$stel.core.observer.longitude * 180) / Math.PI).toFixed(2);
    link += "&elev=" + context.$stel.core.observer.elevation;
    if (!context.$store.state.selectedObject) {
      link +=
        "&az=" +
        ((context.$stel.core.observer.yaw * 180) / Math.PI).toPrecision(5);
      link +=
        "&alt=" +
        ((context.$stel.core.observer.pitch * 180) / Math.PI).toPrecision(5);
    }
    return link;
  },
  ...
  lookupSkySourceByName: function (name) {
    return fetch(
      process.env.VUE_APP_NOCTUASKY_API_SERVER +
        "/api/v1/skysources/name/" +
        name
    ).then(
      function (response) {
        if (!response.ok) {
    ...
    return that
      ...
      .then(
        ...
        () => {
          ...
          return that.lookupSkySourceByName(names[1]).then(
            (res) => {
              return res;
            },
            () => {
              if (names.length === 2) return printErr(names);
              return that.lookupSkySourceByName(names[2]).then(
                (res) => {
                  return res;
                },
                () => {
  ...
  getSkySourceSummaryFromWikipedia: function (ss) {
    ...
    if (ss.model === "constellation") {
      title =
        this.cleanupOneSkySourceName(ss.names[0]).toLowerCase() +
        "_(constellation)";
    }
    if (ss.model === "dso") {
    ...
    if (!title)
      return Promise.reject(new Error("Can't find wikipedia compatible name"));

    return fetch(
      "https://en.wikipedia.org/w/api.php?action=query&redirects&prop=extracts&exintro&exlimit=1&exchars=300&format=json&origin=*&titles=" +
        title,
      ...
      if (response.ok) {
        return response.json().then((res) => {
          const city = res.address.city
            ...
            : res.name;
          loc.short_name =
            pos.accuracy > 500 ? ctx.$t("Near {0}", [city]) : city;
  ...
  getCircumpolarStars: function (obs, minMag, maxMag) {
    ...
    const filter = function (obj) {
      ...
      const posJNOW = $stel.convertFrame(
        obs,
        "ICRF",
        "JNOW",
        obj.getInfo("radec")
