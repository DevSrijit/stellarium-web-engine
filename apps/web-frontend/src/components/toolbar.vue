// Stellarium Web - Copyright (c) 2018 - Noctua Software Ltd // // This program
  ...
  <div id="toolbar-image">
    <v-toolbar class="transparent" dense>
      ...
      <div
        ...
        class="subheader grey--text hidden-sm-and-down pr-2"
        style="user-select: none"
      >
        FPS {{ $store.state.stel ? $store.state.stel.fps.toFixed(1) : "?" }}
      </div>
      <div
        class="subheader grey--text hidden-sm-and-down"
        style="user-select: none"
      >
        FOV {{ fov }}
      </div>
      <v-btn class="transparent" v-if="!$store.state.showSidePanel" to="/p"
        >{{ $t("Observe") }}<v-icon>mdi-chevron-down</v-icon></v-btn
      >
    </v-toolbar>
  </div>
</template>
